...
Last updated: 2026-05-03 (Phase 4.1 Part 1 COMPLETE — wrapper infrastructure built; acceptance test running on user's bin.020 PacBio assembly. Built single-command end-to-end pipeline: `python3 cultureforge.py process --input <genome.fna>` orchestrates prodigal → gapseq → GenomeSPOT → marker BLAST → optional CheckM2/MeBiPred → loads everything into the database under a clean gid >= 1000. **Database cleanup:** removed bad gid=904 entry (E. coli accession with the user's blind-test gapseq data — caused by load_gapseq.py's "idempotent reload" silently dropping gid=32 + reinserting at next auto-increment), then restored gid=32 from the existing E. coli gapseq output (1922 pathways, 2170 transporters, 7 reaction markers, 4 markers from BLAST, 10 GenomeSPOT predictions, 4319 MeBiPred predictions). gid=32 V12 score back to 100% baseline. All 26 test-set genomes + 4 sentinels intact. **New code:** `register_genome.py` (register_genome / deregister_genome / update_genome_metadata; refuses duplicate-accession registration with loud ValueError; refuses to deregister gids < 1000 to protect test set + sentinels — closes the silent-overwrite hole that caused the gid=904 incident). `loaders/` package with thin façades: `gapseq_generic.py`, `genomespot_generic.py`, `marker_blast_generic.py`, `mebipred_generic.py` (all wrap the existing gid-parameterized loader functions; existing `load_*.py` scripts left untouched per Phase 4.1 prohibition). `process_genome.py` orchestration with conda env discovery (`find_conda_env_bin` + `find_tool` look for envs named `gapseq`/`checkm2`/`genomespot` via `conda env list --json`); CULTUREFORGE_<TOOL>_BIN env-var override for non-conda installs; missing-tool error messages naming the install command; optional-tool graceful skip with clear notice; cleanup-on-failure via `deregister_genome` rolls back partial state on any pipeline-stage exception. `cultureforge.py` gets new `process` subcommand with `--input`, `--accession`, `--biomass-template`, `--skip-checkm2`, `--skip-mebipred`, `--inspect` (chain to inspect after process), `--verbose`. **Test-set immutability:** all new genomes go to gid >= 1000 (USER_GID_MIN); test set (7-32) and sentinels (900-903) protected by deregister gid-range guard. **Acceptance test result:** `cultureforge.py process` ran end-to-end on the user's bin.020 PacBio assembly. Stage 1 (prodigal): 1900 proteins predicted ✓. Stage 2 (gapseq, ~3 hr runtime): 1922 pathways + 663 transporters + 7 reaction markers ✓. Stage 3 (GenomeSPOT): 10 envelope predictions (T_opt 23°C, pH 6.8, salinity 2.4% NaCl, oxygen tolerant) ✓. Stage 4 (marker BLAST): 2 markers fired (autotrophy 84.6% pident, terminal_oxidases 44%) ✓. **One bug surfaced and fixed during acceptance test:** the wrapper's `_resolve_gapseq_files` looked for `<accession>-all-Pathways.tbl` based on user's `--accession`, but gapseq names outputs from the staged FASTA stem (`genome-*.tbl`). Fix applied (`process_genome.py` now passes `accession=None` to let the loader glob the directory). Cleanup-on-failure validated: when the load-step file resolution failed, `deregister_genome(gid=1000)` rolled back partial state with zero orphan rows. Replay against the existing gapseq output succeeded; no need to re-run gapseq's 3-hour search. **Predicted recipe:** primary mode `aerobic_chemotrophic` at 0.65 confidence; air atmosphere, 23°C, pH 6.8, glucose+lactose carbon, SL-10 trace metals, Wolin's vitamins; ΔG -2880 kJ/mol (feasible). Functional-neighbor V12 comparison: 67% agreement across 37 reference media (no critical/important diffs; 94 minor concentration-disagreement diffs as expected for novel-lineage organisms without direct DSMZ media). **Full report:** `data/user_genomes/ST3_PacBio_bin20/BLIND_TEST_RESULT.md` includes per-marker hit table, capability detection summary across all 19 supported metabolisms, predicted recipe, functional-neighbor matches, notable observations (gapseq sulfide-oxidation pathway at 100% but soxB marker negative — possible divergent SoxB; salinity prediction didn't trigger halophile mode in the recipe-composer; auxotrophy + cofactor-biosynthesis flags), and a "compare-against-known-conditions" checklist for the user. Second small wrapper improvement during acceptance test: GenomeSPOT-Python discovery now prefers the `genomespot` conda env when present, falls back to ambient Python only if joblib is importable (was crashing with `ModuleNotFoundError` when system Python lacked joblib). README rewritten with realistic installation (apt prodigal + ncbi-blast+; conda gapseq; optional checkm2/mymetal) and the new `process`+`inspect` workflow. See `data/release/phase4_1_cleanup_notes.md` for the gid=904 incident postmortem.)

Earlier (2026-05-02): Phase 3.8 Part 1 COMPLETE: methanogenesis `diagnostic_marker_override` added — symmetric with the Phase 3.5+ override pattern (methanotrophy / DNRA / NOB). Override block on `mcrA` at min_pident=50, min_qcov=70, override_confidence=0.65. Empirical threshold verification: gid=8 Methanocaldococcus mcrA 83.7%, gid=28 Methanoperedens 70.1%, gid=903 Methanosarcina 100% — all fire; zero non-methanogen mcrA cross-reactivity in test set (only those 3 genomes have any mcrA hit). Verification: gid=903 Methanosarcina sentinel NOW classifies methanogenic primary (was escalated/no-primary in Phase 3.7 — closes the marker-only sentinel gap that surfaced in Phase 3.7). gid=8 Methanocaldococcus stays methanogenic primary at confidence 0.73 (gapseq path unchanged; override is additive). gid=28 Methanoperedens stays anme_reverse_methanogenic primary (Phase 3.6 priority ordering correctly supersedes methanogenesis at recipe time). V12 byte-identical to Phase 3.7 baseline across all 25 test organisms — the override is dormant for genomes that already have gapseq pathway data, only kicks in for marker-only sentinels. LIMITATIONS validation-status section updated: methanogenesis moved from "inferred from test-set behavior" to "validated against named-strain sentinel" (Methanosarcina acetivorans C2A). Phase 3.8 Part 2 (documentation polish — README rewrite, consolidated VALIDATION_REPORT, USER_GUIDE_LIMITATIONS, tester onboarding materials, PHASE_3_CLOSEOUT retrospective) follows.)
